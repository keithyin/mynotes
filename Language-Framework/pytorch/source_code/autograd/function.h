...
// 这个 Flag 在 创建 backward functions 的时候会用到
// 标记了这个函数是否可 执行 和 是否为 volatile
// 还保存了 哪些函数用了这个函数 的 输出，这个在反向传导的时候，用于 对于 梯度的 聚集。
...
// Function 还有 pre_hooks, post_hooks ??? 难道这些东西都是注册到 Function 上的？
struct Function : std::enable_shared_from_this<Function> {
  ...
  // 这里重载了 函数调用运算符， 可以直接把对象当作函数调用，cosy 的很
