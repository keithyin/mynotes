...
// 编译器会对上面的程序进行程序转化， 
// 有两个阶段： 1. 重写每一个定义， 其中的初始化操作被剥除。2. class 的构造函数调用会被安插进去
// 以下是转化后的代码
void foo_bar() {
 	X x1; //定义被编译器重写了，初始化操作被删除
  X x2; //定义被编译器重写了，初始化操作被删除
  X x3;//定义被编译器重写了，初始化操作被删除
...
// foo 会被转换为 foo(X &x0)
foo(__tmp0);
__tmp0.X::~X(); // 这里会插入 析构函数调用， 在 foo 函数外
...
* 两阶段转化
  * 首先函数加一个额外的参数，类型是返回值的 reference
  * 在return之前安插一个copy constructor调用操作，以便于将欲传回的object的内容 当做新增参数的初值。
...
* 初始化列表初始化执行的顺序：**变量在类中声明的顺序**
* 能初始化列表中初始化，就在初始化列表中初始化
