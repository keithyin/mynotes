...
auto makeFlags(const T &inputs) -> FunctionFlags {
  ...
  f.next_functions.resize(num_inputs);
  {
    ...
    for (auto it = inputs.begin(); it != inputs.end(); ++it, ++i) {
      auto& var = *it; // var 存的是 引用 *it,解指针
      if (var.defined()) {
        f.is_executable |= var.requires_grad(); //如果有任何一个 var.requires_grad=true, 则 f.is_executable=true
        f.is_volatile |= var.is_volatile(); //如果有任何一个 var.is_volatile()=true, 则 f.is_volatile=true
        if (var.grad_fn()) { // 如果 var 不是叶子节点，？？？？ 这个地方很瞢！！！！ 需要研究下 Variable
  ...
  f.is_executable &= !f.is_volatile; // 如果 f.is_volatile=true 则 f.is_executable=false
  return f;
}
