...
class RoiPoolOp : public OpKernel {
 public:
  ...
  void Compute(OpKernelContext* context) override
  {
    ...
    // 这个地方设置了 输出的维度， dims[0] 是 roi 的数量，而不是 batch 的数量。
    ...
    auto shard = [pooled_height, pooled_width, spatial_scale,
      ...
      for (int64 b = start; b < limit; ++b)
      {
        ...
        // 别看展平的是一维， 更改的依旧是之前的Tensor
        /*********************************************************/
		
        // 找到第 n 个 样本的 roi 起始位置。
        const float* bottom_rois = bottom_rois_flat.data() + n * 5;
        
        // batch_index? 难道是 指明 当前roi 是属于哪个样本的？
        // 的确是用来指明当前 roi 是属于哪个样本的。
...
> *输入为： 前向过程中的输入，还有下一层传过来的梯度*
>
> *输出为：本层输入的梯度。*
>
> *纠结的地方是：tensorflow 是如何 将计算的梯度，与输入对应上的。。怪异*
>
> *REGISTER_OP 的时候，命名有啥讲究吗？*
...
class RoiPoolGradOp : public OpKernel {
 public:
  ...
  void Compute(OpKernelContext* context) override
  {
    ...
    // 前向过程中的后一层 传过来的梯度，即此层的输出的梯度。
    const Tensor& out_backprop = context->input(3);
	// 求梯度的过程，只需要找到 argmax_data 然后把对应的梯度
    // 放回去就好了。
...
*`Tensorflow`*  `API` 具有一个特性，就是 `shape inference`：不用执行计算图，就可以获得 `tensor` 的`shape` 信息。这种功能是由 注册 op 时使用的 `SetShapeFn` 提供的：
...
REGISTER_OP("ZeroOut")
    ...
    .SetShapeFn([](::tensorflow::shape_inference::InferenceContext* c) {
      c->set_output(0, c->input(0)); //这是一个 匿名函数，用来设置输出的 shape。
      return Status::OK();
    });
```
